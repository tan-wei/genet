...
class Notifier extends EventEmitter {
  constructor (type, handle, disposer, sess) {
    super()
    this._disposable = new Disposable(disposer)
    this._sess = sess
    this._listener = (event) => {
      if (event.id === handle && event.type === type) {
        if (event.error === null) {
          this.emit('finish')
        } else {
    ...
    this._sess.on('update', this._listener)
  }

  dispose () {
    this._sess.removeListener('update', this._listener)
...
class Session extends EventEmitter {
  ...
  createReader (id, arg = {}) {
    ...
    return new Notifier('input', handle, () => {
      this._sess.closeReader(handle)
    }, this)
  }

  createWriter (id, arg = {}, filter = '') {
    const filterCompiler = new FilterCompiler()
    filterCompiler.macros = Array.from(this._options.filterMacros)
    const body = filterCompiler.compile(filter, { built: false }).linked
    ...
    return new Notifier('output', handle, () => {
