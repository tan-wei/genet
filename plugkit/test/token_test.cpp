...
TEST_CASE("Token_get", "[Token]") {
  CHECK(Token_from(nullptr) == Token_null());
...
TEST_CASE("Token_string", "[Token]") {
  CHECK(Token_from(Token_string(Token_from(nullptr))) == Token_from(nullptr));
  CHECK(Token_from(Token_string(Token_get(""))) == Token_get(""));
  CHECK(Token_from(Token_string(Token_get("a"))) == Token_get("a"));
  CHECK(Token_from(Token_string(Token_get("eth"))) == Token_get("eth"));
  CHECK(Token_from(Token_string(Token_get("[eth]"))) == Token_get("[eth]"));
  CHECK(Token_from(Token_string(Token_get("ipv4"))) == Token_get("ipv4"));
  CHECK(Token_from(Token_string(Token_get("[ipv4]"))) == Token_get("[ipv4]"));
