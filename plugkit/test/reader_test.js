...
describe('Reader', () => {
  describe('#getUint8()', () => {
    it('should return a Uint8 value', () => {
      ...
      assert.equal(200, reader.getUint8())
      assert.equal(250, reader.getUint8())
      assert.equal(255, reader.getUint8())
      assert.equal(254, reader.getUint8())
  ...
  describe('#getInt8()', () => {
    it('should return an Int8 value', () => {
      const reader = new Reader()
      reader.data = Testing.externalize(new Uint8Array([200, 250, -1, -2]))
      assert.equal(-56, reader.getInt8())
      assert.equal(-6, reader.getInt8())
      assert.equal(-1, reader.getInt8())
      assert.equal(-2, reader.getInt8())
  ...
  describe('#lastError', () => {
    it('should return reader last error', () => {
      ...
      assert.equal('.dst', reader.lastError.target)
      assert.equal('Invalid value', reader.lastError.type)
    })
  })
})
