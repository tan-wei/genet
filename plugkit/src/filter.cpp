...
public:
  v8::UniquePersistent<v8::Function> filterFunc;
  FilterFunc func;
};
...
Filter::Filter(const std::string &body) : d(new Private()) {
  ...
  if (!script.IsEmpty()) {
    ...
    if (!result.IsEmpty()) {
      ...
      if (func->IsFunction()) {
        d->filterFunc.Reset(v8::Isolate::GetCurrent(), func.As<v8::Function>());
...
bool Filter::test(const FrameView *view) const {
  ...
  auto global = isolate->GetCurrentContext()->Global();
  auto func = v8::Local<v8::Function>::New(isolate, d->filterFunc);
  if (!func.IsEmpty()) {
    v8::Local<v8::Value> args[1] = {FrameWrapper::wrap(view)};
