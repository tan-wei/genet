...
lazy_static! {
    ...
    static ref TYPE_MAP: HashMap<u64, (Token, Ptr<AttrClass>)> = hashmap!{
        0x0800 => (token!("ipv4"), AttrBuilder::new("eth.type.ipv4").decoder(decoder::Const(true)).build()),
        0x0806 => (token!("arp"), AttrBuilder::new("eth.type.arp").decoder(decoder::Const(true)).build()),
        0x0842 => (token!("wol"), AttrBuilder::new("eth.type.wol").decoder(decoder::Const(true)).build()),
        0x86DD => (token!("ipv6"), AttrBuilder::new("eth.type.ipv6").decoder(decoder::Const(true)).build()),
        0x888E => (token!("eap"), AttrBuilder::new("eth.type.eap").decoder(decoder::Const(true)).build()),
    };
}
