...
impl Worker for IPv6Worker {
    fn decode(
        ...
        let nheader = NHEADER_ATTR_HEADER.try_get(&layer.data())?.try_into()?;
        ...
        let proto = proto_attr.try_get(&layer.data())?.try_into()?;
...
def_layer_class!(IPV6_CLASS, "ipv6",
    ...
    header: attr!(&TRAFFIC_ATTR, range: 0..2),
    header: attr!(&FLOW_ATTR, range: 1..4),
