  ...
  view(vnode) {
    return m('main', [
      m('section', [
        ...
        m('p', [
          m('b', ['TL;DR:']), ' macOS does not allow normal users to access pcap devices by default.' + ' ' + 'Deplug provides a tiny helper tool to handle this problem.' + ' ' + 'Once it has been installed, we can start a live capture without running Deplug as root.'
        ]),
      ...
      m('section', [
        ...
        m('ol', [
          m('li', ['sudo launchctl unload /Library/LaunchDaemons/net.deplug.DeplugHelper.plist']),
          m('li', ['sudo rm /Library/LaunchDaemons/net.deplug.DeplugHelper.plist']),
          m('li', ['sudo rm /Library/PrivilegedHelperTools/net.deplug.DeplugHelper']),
