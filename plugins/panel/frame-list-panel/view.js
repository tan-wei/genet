import { Channel, Profile, Renderer } from 'deplug'
import m from 'mithril'
import throttle from 'lodash.throttle'

class FrameItem {
  oninit (vnode) {
    [this.frame] = vnode.attrs.session.getFrames(vnode.attrs.seq - 1, 1)
  }
  select () {
  ...
  view (vnode) {
    ...
    return m('div', {
      ...
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.index')]),
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.id')]),
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.src')]),
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.dst')]),
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.length')]),
      m('div', { class: 'frame-column' },
        [Renderer.query(this.frame, '.primaryLayer')])
    ])
  }
}
export default class FrameListView {
  ...
  onupdate (vnode) {
    ...
    const maxScrollTop = this.frameView.scrollHeight -
    this.frameView.clientHeight
  ...
  updateMap (vnode) {
    if (this.session && this.frame.frames > 0) {
      ...
      const frames = (this.filtered
        ? this.filtered.frames
        : this.frame.frames)
        ...
        if (frames > 0) {
          for (let line = 0; line < this.mapHeight; line += 1) {
            ...
            const [frame] = this.session.getFrames(index, 1)
    ...
    view () {
      const frames = (this.filtered
        ...
        const margin = 5
        const begin = Math.max(0, Math.floor(
          Math.min(this.viewScrollTop, viewHeight - this.viewHeight) /
          itemHeight) - margin)
          const end = Math.min(begin +
            ...
            if (this.filtered) {
              filterdFrames =
              this.session.getFilteredFrames('main', begin, (end - begin))
            }
            return [
              ...
              m('div', { class: 'header' }, [
                m('div', { class: 'frame-column' }, ['No.']),
                m('div', { class: 'frame-column' }, ['Protocol']),
                m('div', { class: 'frame-column' }, ['Source']),
                m('div', { class: 'frame-column' }, ['Destination']),
                m('div', { class: 'frame-column' }, ['Length']),
                m('div', { class: 'frame-column' }, ['Summary'])
              ]),
              m('div', { class: 'frame-list-view' }, [
                m('div', {
                  class: 'padding',
                  style: { height: `${viewHeight}px` },
                }, [
                  (new Array(end - begin)).fill()
                  .map((dev, index) => {
                    const id = filterdFrames
                    ? filterdFrames[index]
                    : (index + begin + 1)
                    const selected = this.selectedFrames
                    .some((frame) => frame.index === id)
                    return m(FrameItem, {
                      key: id,
                      seq: id,
                      index: index + begin,
