...
export default class FrameListView {
  ...
  onupdate(vnode) {
    ...
    if (this.previousScrollTop <= this.frameView.scrollTop) {
      ...
      this.previousScrollTop = Math.max(this.previousScrollTop, this.frameView.scrollHeight * 0.8)
    }
  }
  updateMap(vnode) {
  ...
  view(vnode) {
    ...
    const margin = 5
    const begin = Math.max(0, Math.floor(Math.min(this.viewScrollTop, viewHeight - this.viewHeight) / itemHeight) - margin)
    const end = Math.min(begin + Math.ceil(this.viewHeight / itemHeight) + margin * 2, frames)
